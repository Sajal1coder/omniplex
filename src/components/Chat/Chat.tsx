...
const Chat = (props: Props) => {
    ...
    const processChatThread = async () => {
      if (chatThread && chatThread.chats.length > 0) {
        ...
        if (lastChatId !== lastProcessedChatRef.current) {
          ...
          if(lastChat.mode==="linkedin" && !lastChat.linkedinResults){
            try {
              console.log("lastchat.arg.email",lastChat.arg.email);
              await handleLinkedIn(lastChat.arg.email,lastChatIndex);
            } catch (error) {
              setError("Error fetching or proceesing search results");
              setErrorFunction(() =>
                handleLinkedIn.bind(null,lastChat.arg.email,lastChatIndex)
              );
              return;
            }
          }
  ...
  const handleWeather = async (location: string, chatIndex: number) => {
   ...
   const handleLinkedIn=async(email:string,chatIndex:number) =>{
    const chat=chatThread?.chats[chatIndex];
    setIsLoading(true);
    setIsCompleted(false);
    try{
      if (chat?.mode === "linkedin") {
        ...
        dispatch(updateLinkedin({
          threadId: id,
          chatIndex,
          linkedinResults: linkedinData,
        }));
        setError("");
        await handleAnswer(chat, JSON.stringify(linkedinData));
      } else {
        throw new Error("Mode is not LinkedIn");
      }
    }  catch (error) {
        console.error("Error fetching or processing linkedin data:", error);
        setError("Error fetching or processing linkedin data");
        setErrorFunction(() => handleLinkedIn.bind(null, email, chatIndex));
