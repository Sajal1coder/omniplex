...
const chatSlice = createSlice({
  ...
  reducers: {
    ...
    updateLinkedin:(
      ...
      const { threadId, chatIndex, linkedinResults } = action.payload;
      if (state.threads[threadId]) {
        // Check if the chats array exists and the chatIndex is valid
        if (state.threads[threadId].chats && state.threads[threadId].chats[chatIndex]) {
          state.threads[threadId].chats[chatIndex].linkedinResults = linkedinResults;
        } else {
          console.error(`Chat index ${chatIndex} does not exist for thread ${threadId}`);
        }
      } else {
        console.error(`Thread ID ${threadId} does not exist`);
      }
      state.threads[threadId].chats[chatIndex].linkedinResults = linkedinResults;
